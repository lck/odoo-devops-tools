...
_LOCAL_DOCKER_DB_CONNECTION_CONFIG_KEYS = {"db_host", "db_port", "db_name", "db_user", "db_password"}
_LOCAL_DOCKER_ODOO_CONF_IGNORED_CONFIG_KEYS = {
    "addons_path",
    "data_dir",
    *_DOCKER_HOST_PORT_CONFIG_KEYS,
    *_LOCAL_DOCKER_DB_CONNECTION_CONFIG_KEYS,
}
...
def load_project_config(
    ...
    if cp.has_section("docker"):
        supported_docker_options = {"base_image", "compose_project_name", "db_service", "odoo_service"}
        for key in cp._sections.get("docker", {}).keys():
            if key == "addons_mode":
                raise Exception(
                    "Option 'addons_mode' in section [docker] has been removed. "
                    "Local Docker artifacts are generated by default; use --create-docker-deploy "
                    "to generate the self-contained deploy context."
                )
            if key not in supported_docker_options:
                raise Exception(
                    f"Unsupported option '{key}' in section [docker]. "
                    "Supported options: base_image, compose_project_name, db_service, odoo_service."
                )
...
def _docker_requirements_lock_path(docker_context_dir: Path) -> Path:
    return _docker_requirements_dir(docker_context_dir) / "addons-requirements.lock.txt"
...
def compile_docker_addons_requirements_lock(
    ...
    """Compile an addon-only requirements lock for a generated Docker image."""
    _ensure_command("uv")
...
def render_local_docker_odoo_conf(cfg: ProjectConfig) -> str:
    """Render Odoo config for the local Docker Compose runtime."""
    lines: list[str] = ["[options]"]

    # Host-side DB connection settings belong to the non-Docker workspace. The
    # local container uses the Compose service/environment connection instead.
    for key, value in cfg.config.items():
        if key in _LOCAL_DOCKER_ODOO_CONF_IGNORED_CONFIG_KEYS:
            continue
        lines.append(f"{key} = {_format_conf_value(value)}")

    lines.append(f"addons_path = {_docker_local_container_addons_path(cfg)}")
    lines.append("data_dir = /var/lib/odoo")

    return "\n".join(lines) + "\n"


def write_local_docker_odoo_conf(layout: Layout, cfg: ProjectConfig) -> Path:
    path = _docker_odoo_conf_path(layout.docker_local_dir)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(render_local_docker_odoo_conf(cfg), encoding="utf-8")
    return path
...
def _prepare_docker_requirements(
    ...
    if copy_from is not None:
        ...
        lock_path = _docker_requirements_lock_path(docker_context_dir)
        build_constraints_path = _docker_build_constraints_path(docker_context_dir)
        if not lock_path.is_file():
            raise Exception(f"Copied Docker requirements are missing lock file: {lock_path}")
        return lock_path, build_constraints_path
...
def create_local_docker_artifacts(
    ...
    lock_path, build_constraints_path = _prepare_docker_requirements(
        layout=layout,
        cfg=cfg,
    ...
    docker_conf_path = write_local_docker_odoo_conf(layout, cfg)
    ...
    return {
        ...
        "requirements_input": _docker_requirements_input_path(layout.docker_local_dir),
...
def create_deploy_docker_artifacts(
    ...
    return {
        ...
        "requirements_input": _docker_requirements_input_path(layout.docker_deploy_dir),
        "requirements_lock": lock_path,
        "build_constraints": build_constraints_path if build_constraints_path.is_file() else None,
...
def _sync_project_impl(
    ...
    if local_docker_artifacts is not None:
        ...
        docker_lock_path = _docker_requirements_lock_path(layout.docker_local_dir)
    ...
    if deploy_docker_artifacts is not None:
        ...
        deploy_lock_path = _docker_requirements_lock_path(layout.docker_deploy_dir)
        if deploy_lock_path.exists():
            _logger.info(f"  Deploy Requirements: {deploy_lock_path}")
